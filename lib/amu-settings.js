    ...
    apply() {
        ...
        atom.config.onDidChange('atom-material-ui.colors.accentColor', (value) => {
            ...
            if (!accentColor.isValid()) {
                return atom.notifications.addError(`The selected color <code>${value.newValue.toHexString()}</code> is not a valid HEX value. Try another!`, { dismissable: true });
            }
            amu.writeConfig();
        });
        ...
        atom.config.onDidChange('atom-material-ui.colors.predefinedColor', (value) => {
            // FIXME: This piece of code looks too damn wet. However, there doesn't seem to be
            // a way to use custom values in <select> elements and it only picks up the <option>
            // text node as a value. Bummer :(
