...
export default {
    ...
    writeConfig(options) {
        var accentColor = atom.config.get('learn-ide-material-ui.colors.accentColor').toRGBAString();
        var baseColor = atom.config.get('learn-ide-material-ui.colors.abaseColor').toRGBAString();
        ...
        */
        ...
        fs.writeFile(`${__dirname}/../styles/custom.less`, config, 'utf8', () => {
            if (!options || !options.noReload) {
                var themePack = atom.packages.getLoadedPackage('learn-ide-material-ui');
